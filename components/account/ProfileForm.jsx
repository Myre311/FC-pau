...
export function ProfileForm({ user }) {
  ...
  return (
    <form action={action} className="space-y-6">
      ...
      <label className="flex items-start gap-3 border-t border-white/10 pt-5">
        <input
          type="checkbox"
          name="newsletterOptIn"
          defaultChecked={user.newsletterOptIn}
          className="mt-1 h-4 w-4 flex-none border-white/30 bg-transparent accent-jaune"
        />
        <span>
          <span className="block font-mono text-[11px] uppercase tracking-[0.2em] text-white">
            Newsletter Pau FC
          </span>
          <span className="mt-1 block font-sans text-sm text-white/60">
  ...
  return (
    <label className="block">
      <span className="font-mono text-[10px] uppercase tracking-[0.2em] text-white/50">
        {label}
        {required && !disabled && <span className="ml-1 text-pau-yellow">*</span>}
      </span>
      <input
        type={type}
        name={name}
        ...
        disabled={disabled}
        className="mt-2 block h-11 w-full border border-white/15 bg-transparent px-3 font-sans text-sm text-white outline-none transition-colors focus:border-pau-yellow disabled:opacity-50"
      />
      {hint && (
